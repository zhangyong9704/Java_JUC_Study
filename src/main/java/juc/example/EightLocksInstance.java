...
/**
 * 内容摘要:
 * <p>文件名称: EightLocksInstance.java
 ...
 *      锁的是当前对象this(当前new出来的对象实例)，被锁定后，其他的线程都不能进入到当前对象的其他的synchronized方法
...
public class EightLocksInstance {
...
class Phone{
    //静态同步方法块  与所有使用Class的静态同步方法互斥
    public static synchronized void sendEmail() throws Exception{
        try { TimeUnit.SECONDS.sleep(3); } catch (InterruptedException e) { e.printStackTrace(); }
        System.out.println(Thread.currentThread().getName()+"*******sendEmail");
    }
    //非静态同步方法快  与同一个对象内的其他静态同步方法互斥
    public synchronized void sendMs() throws Exception{
        TimeUnit.SECONDS.sleep(2);
        System.out.println(Thread.currentThread().getName()+"*******sendMs");
    }

    public synchronized void sendMs2() throws Exception{
        TimeUnit.SECONDS.sleep(2);
        System.out.println(Thread.currentThread().getName()+"*******sendMs2");
    }
